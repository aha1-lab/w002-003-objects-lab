...
function changeDiffeculities(level){
    for (let i = 0; i < game.gyms.length; i++) {
        const element = game.gyms[i];
        if(element.difficulty < level){
...
game.gymStatus = () =>{
    ...
    game.gyms.forEach((gym)=>{
        if(gymTally[gym.completed]){
            gymTally[gym.completed] = gymTally[gym.completed] +1;
        }else{
            gymTally[gym.completed] = 1
        }
    })

    console.log(`{ completed: ${gymTally.true}, incomplete: ${gymTally.false} }`)
}
